...
export default function App() {
  ...
  const getRankColor = (index) => {
    if (index === 0) return 'text-amber-500 font-black'; // Gold
    if (index === 1) return 'text-zinc-300 font-extrabold'; // Silver
    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-200 p-4 font-sans selection:bg-zinc-800 flex flex-col justify-between">
      <div className="max-w-4xl mx-auto w-full flex-1">
        ...
        {view === 'all' && (
          <div className="space-y-4">
            ...
            {/* Search Input Bar */}
            <div className="relative flex items-center bg-[#18181b]/40 border border-zinc-800/80 rounded-xl px-3 py-2 text-zinc-400 focus-within:border-zinc-700 focus-within:text-zinc-200 transition-all">
                          ...
                          <div className="flex items-center gap-2 flex-wrap h-5">
                            {album.spotify_url ? (
                              <a 
                                ...
                                className="text-base font-bold text-zinc-100 hover:text-emerald-400 hover:underline transition leading-none"
                              >
                                {album.title}
                              </a>
                            ) : (
                      ...
                      <div className="flex items-center gap-4 shrink-0">
                        ...
                        {isAdmin && (
                          <div className="flex gap-1">
                            <button onClick={(e) => startEditing(e, album)} className="p-1.5 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-white transition">
                              <Pencil size={12} />
                            </button>
                            <button onClick={(e) => deleteAlbum(e, album.id)} className="p-1.5 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-red-400 transition">
                              <Trash2 size={12} />
                            </button>
                          </div>
                        )}
              ...
              {rankSubTab === 'albums' ? rankedAlbums.map((album, i) => {
                ...
                return (
                  <div key={album.id} className="relative bg-[#18181b]/40 p-4 rounded-xl border border-zinc-900 shadow-md flex items-center justify-between overflow-hidden group hover:border-zinc-800/60 transition">
                        ...
                        <div className="flex items-baseline gap-2 flex-wrap h-5">
                          {album.spotify_url ? (
                            <a 
                              ...
                              target="_blank" 
                              rel="noreferrer"
                              className="text-sm font-bold text-zinc-100 hover:text-emerald-400 hover:underline transition leading-none"
                            >
                              {album.title}
                            </a>
                          ) : (
                    ...
                    <div 
                      className="absolute bottom-0 left-12 h-[2px] bg-amber-500/40 rounded-full transition-all duration-500 group-hover:bg-amber-500/70"
                ...
                return (
                  <div key={song.id} className="relative bg-[#18181b]/40 p-4 rounded-xl border border-zinc-900 shadow-md flex items-center justify-between overflow-hidden group hover:border-zinc-800/60 transition">
                    ...
                    <div className="text-xl font-bold text-zinc-200 shrink-0 pl-4 z-10">
      ...
      <div className="max-w-4xl mx-auto w-full border-t border-zinc-900/80 mt-12 pt-6 pb-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
        ...
        <div className="text-zinc-600 font-medium tracking-wide text-center sm:text-left">
          Built with ⚡ by <span className="text-zinc-400 font-bold hover:text-white transition cursor-default">Mukund</span>
        </div>
          ...
          {isAdmin ? (
            <div className="flex items-center gap-3 bg-zinc-900/40 border border-zinc-800/50 px-3 py-1.5 rounded-xl">
              ...
              <button 
                ...
                className="text-[10px] text-zinc-400 font-bold tracking-wider hover:text-white ml-2 bg-zinc-800 border border-zinc-700/60 px-2 py-0.5 rounded-lg transition"
              >
                LOCK
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50" onClick={closeModal}>
          <div className="bg-[#18181b] w-full max-w-lg p-6 rounded-3xl border border-zinc-800/80 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {!editingAlbumId && (
              <div className="mb-4 flex gap-2 bg-zinc-950 p-2 rounded-xl border border-zinc-800/60 focus-within:border-zinc-700 transition">
                ...
                <button 
                  ...
                  className="bg-zinc-900 border border-zinc-700 text-[10px] font-bold text-zinc-300 hover:text-white px-3 py-1 rounded-lg disabled:opacity-40 transition flex items-center gap-1 shrink-0"
                >
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <input placeholder="Album title" className="bg-zinc-900/60 border border-zinc-800/80 p-2.5 rounded-xl outline-none focus:border-zinc-600 text-sm text-white" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
              <input placeholder="Artist" className="bg-zinc-900/60 border border-zinc-800/80 p-2.5 rounded-xl outline-none focus:border-zinc-600 text-sm text-white" value={formData.artist} onChange={e => setFormData({...formData, artist: e.target.value})} />
            </div>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <input placeholder="Year" type="number" className="bg-zinc-900/60 border border-zinc-800/80 p-2.5 rounded-xl outline-none focus:border-zinc-600 text-sm text-white" value={formData.year} onChange={e => setFormData({...formData, year: e.target.value})} />
              <select className="bg-zinc-900/60 border border-zinc-800/80 p-2.5 rounded-xl outline-none focus:border-zinc-600 text-zinc-400 text-sm" value={formData.genre} onChange={e => setFormData({...formData, genre: e.target.value})}>
                {['Pop', 'Hip Hop', 'Rock', 'R&B', 'Electronic', 'Country'].map(g => <option key={g} value={g}>{g}</option>)}
              </select>
            </div>
            <div className="grid grid-cols-1 gap-3 mb-3">
              <input placeholder="Image Cover URL (Optional)" className="w-full bg-zinc-900/60 border border-zinc-800/80 p-2.5 rounded-xl outline-none focus:border-zinc-600 text-xs text-zinc-400" value={formData.image_url} onChange={e => setFormData({...formData, image_url: e.target.value})} />
            </div>
            <div className="mb-4">
              <input placeholder="Spotify Link (Optional)" className="w-full bg-zinc-900/60 border border-zinc-800/80 p-2.5 rounded-xl outline-none focus:border-zinc-600 text-xs text-zinc-400" value={formData.spotify_url} onChange={e => setFormData({...formData, spotify_url: e.target.value})} />
            </div>
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                  {formData.songs.map((song, index) => (
                    <div key={song.id || index} className="flex gap-2">
                      <input className="flex-1 bg-zinc-950 border border-zinc-800 p-2 rounded-lg text-xs outline-none focus:border-zinc-700 text-white" value={song.name} onChange={(e) => {
                        const newSongs = [...formData.songs];
                        newSongs[index].name = e.target.value;
                        setFormData({...formData, songs: newSongs});
                      }} />
                      <button onClick={() => setFormData({...formData, songs: formData.songs.filter((_, i) => i !== index)})} className="bg-red-950/40 text-red-400 w-8 rounded-lg hover:bg-red-500 hover:text-white transition text-xs text-center flex items-center justify-center">✕</button>
                    </div>
                  ))}
                </div>
              </div>
            ) : (
              <textarea placeholder="Tracklist (one per line)" className="w-full bg-zinc-900/60 border border-zinc-800/80 p-3 rounded-xl h-32 outline-none focus:border-zinc-600 mb-4 resize-none text-sm text-white" value={formData.tracks} onChange={e => setFormData({...formData, tracks: e.target.value})} />
            )}
